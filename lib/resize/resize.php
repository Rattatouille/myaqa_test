...
class thumbnail
{
	...
	function size_height($size=100)
	{
		if($this->img["tinggi"] > $size) {
			// don't resize if thumbnail is allready smaller
			if(!isset($this->img["tinggi_thumb"]) || $this->img["tinggi_thumb"] > $size) {
				//height
	...
	function size_width($size=100)
	{
		if($this->img["lebar"] > $size) {
			// don't resize if thumbnail is allready smaller
			if(!isset($this->img["lebar_thumb"]) || $this->img["lebar_thumb"] > $size) {
				//width
